...
function BlockExplorer() {
  ...
  // Load transactions from multiple blocks to show scrollable history
  const loadTransactions = useCallback(async () => {
    ...
    try {
      ...
      // Get recent blocks (load more blocks to show more transactions)
      // This allows users to scroll through transaction history
      const recentBlocks = [];
      const blocksToLoad = 50; // Load last 50 blocks to get more transactions
      for (let i = 0; i < blocksToLoad && blockNumber - i >= 0; i++) {
        try {
      ...
      // Sort by block number (newest first) - no limit, show all transactions
      ...
      setTransactions((prevTx) => {
        ...
        return txList; // Return all transactions, no limit
      });
    } catch (err) {
